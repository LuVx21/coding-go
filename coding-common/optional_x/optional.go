...
func (op Optional[T]) IfPresent(consumer Consumer[T]) {
    if op.IsPresent() {
        consumer(op.data)
    }
}

func (op Optional[T]) IfPresentOrElse(consumer Consumer[T], run Runnable) {
    if op.IsPresent() {
        consumer(op.data)
    } else {
        run()
...
func (op Optional[T]) Filter(predicate Predicate[T]) Optional[T] {
    if op.IsEmpty() {
        return op
    }
...
func (op Optional[T]) Map(mapper Function[T, T]) Optional[T] {
    if op.IsEmpty() {
        return Empty[T]()
    }
    return OfNullable[T](mapper(op.data))
}

func (op Optional[T]) FlatMap(mapper Function[T, Optional[T]]) Optional[T] {
    if op.IsEmpty() {
        return Empty[T]()
    }
...
func (op Optional[T]) Or(supplier Supplier[Optional[T]]) Optional[T] {
    if op.IsPresent() {
        return op
    }
    return supplier()
...
func (op Optional[T]) OrElseGet(supplier Supplier[T]) T {
    if op.IsPresent() {
        return op.data
    }
    return supplier()
